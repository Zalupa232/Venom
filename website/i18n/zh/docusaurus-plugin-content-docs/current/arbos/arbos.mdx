...
ArbOS provides L2-specific precompiles with methods smart contracts can call the same way they can solidity functions. This section documents the infrastructure that makes this possible. For more details on specific calls, please refer to the [methods documentation](./precompiles.mdx).
...
A Retryable is a special message type for creating atomic L1 to L2 messages; for details, see [L1 To L2 Messaging](./l1-to-l2-messaging.mdx).
...
Much of ArbOS's state exists to facilitate its [precompiles](precompiles.mdx). The parts that aren't are detailed below.
